...
func (scaleSet *ScaleSet) GetScaleSetSize() (int64, error) {
	...
	set, err := scaleSet.manager.virtualMachineScaleSetsClient.Get(resourceGroup, scaleSet.Name)
	if err != nil {
		return -1, err
	}
...
func (scaleSet *ScaleSet) SetScaleSetSize(size int64) error {
	resourceGroup := scaleSet.manager.config.ResourceGroup
	op, err := scaleSet.manager.virtualMachineScaleSetsClient.Get(resourceGroup, scaleSet.Name)
	if err != nil {
		return err
	}
	...
	_, errChan := scaleSet.manager.virtualMachineScaleSetsClient.CreateOrUpdate(resourceGroup, scaleSet.Name, op, cancel)
	return <-errChan
}
...
func (scaleSet *ScaleSet) GetScaleSetVms() ([]compute.VirtualMachineScaleSetVM, error) {
	...
	resourceGroup := scaleSet.manager.config.ResourceGroup
	result, err := scaleSet.manager.virtualMachineScaleSetVMsClient.List(resourceGroup, scaleSet.Name, "", "", "")
	if err != nil {
	...
	for moreResults {
		...
		result, err = scaleSet.manager.virtualMachineScaleSetVMsClient.ListNextResults(result)
		if err != nil {
			return nil, err
		}
		moreResults = (result.Value != nil && len(*result.Value) > 0)
...
func (scaleSet *ScaleSet) DeleteInstances(instances []*azureRef) error {
	...
	cancel := make(chan struct{})
	resourceGroup := scaleSet.manager.config.ResourceGroup
	_, errChan := scaleSet.manager.virtualMachineScaleSetsClient.DeleteInstances(resourceGroup, commonAsg.Id(), *requiredIds, cancel)
	return <-errChan
}
