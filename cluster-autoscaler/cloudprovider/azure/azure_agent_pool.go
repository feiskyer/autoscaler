...
func (as *AgentPool) initialize() error {
	deploy, err := as.manager.deploymentsClient.Get(as.manager.config.ResourceGroup, as.manager.config.Deployment)
	if err != nil {
		glog.Errorf("deploymentsClient.Get(%s, %s) failed: %v", as.manager.config.ResourceGroup, as.manager.config.Deployment, err)
		return err
	}

	template, err := as.manager.deploymentsClient.ExportTemplate(as.manager.config.ResourceGroup, as.manager.config.Deployment)
	if err != nil {
...
func (as *AgentPool) preprocessParameters() {
	...
	// etcd TLS parameters (only for acs-engine > v0.11.0)
...
func (as *AgentPool) IncreaseSize(delta int) error {
	...
	_, errChan := as.manager.deploymentsClient.CreateOrUpdate(as.manager.config.ResourceGroup, newDeploymentName, newDeployment, cancel)
...
func (as *AgentPool) GetVirtualMachines() (instances []compute.VirtualMachine, err error) {
	result, err := as.manager.virtualMachinesClient.List(as.manager.config.ResourceGroup)
	if err != nil {
		return nil, err
	}
	...
	for moreResult {
		...
		if result.NextLink != nil {
			result, err = as.manager.virtualMachinesClient.ListNextResults(result)
			if err != nil {
...
func (as *AgentPool) deleteBlob(accountName, vhdContainer, vhdBlob string) error {
	storageKeysResult, err := as.manager.storageAccountsClient.ListKeys(as.manager.config.ResourceGroup, accountName)
	if err != nil {
		return err
	}
...
func (as *AgentPool) deleteVirtualMachine(name string) error {
	vm, err := as.manager.virtualMachinesClient.Get(as.manager.config.ResourceGroup, name, "")
	if err != nil {
	...
	_, deleteErrChan := as.manager.virtualMachinesClient.Delete(as.manager.config.ResourceGroup, name, nil)
	...
	if len(nicName) > 0 {
		...
		_, nicErrChan := as.manager.interfacesClient.Delete(as.manager.config.ResourceGroup, nicName, nil)
	...
	if vhd != nil {
		...
		if osDiskName == nil {
			...
			_, diskErrChan := as.manager.disksClient.Delete(as.manager.config.ResourceGroup, *osDiskName, nil)
