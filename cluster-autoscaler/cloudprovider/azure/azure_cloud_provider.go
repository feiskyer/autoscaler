...
func BuildAzureCloudProvider(azureManager *AzureManager, specs []string, resourceLimiter *cloudprovider.ResourceLimiter) (cloudprovider.CloudProvider, error) {
	azure := &AzureCloudProvider{
		azureManager:    azureManager,
		resourceLimiter: resourceLimiter,
	}
	for _, spec := range specs {
		asg, err := azureManager.buildAsgFromSpec(spec)
		if err != nil {
			return fmt.Errorf("failed to parse node group spec: %v", err)
		}
		if azureManager.RegisterAsg(asg) {
			changed = true
		}

		if changed {
			if err := azureManager.regenerateCache(); err != nil {
				return err
			}
		}
		return nil
